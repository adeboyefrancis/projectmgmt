...
export default function StatsGrid() {
    ...
    useEffect(() => {
        // Added check for projects and owner to prevent crashes if data is missing
        if (currentWorkspace && currentWorkspace.projects && currentWorkspace.owner) {
            setStats({
                ...
                completedProjects: currentWorkspace.projects
                    ...
                    // Added ?. to tasks in case a project has no tasks array
                    .reduce((acc, project) => acc + (project.tasks?.length || 0), 0),
                myTasks: currentWorkspace.projects.reduce(
                    (acc, project) =>
                        acc +
                        (project.tasks?.filter(
                            // Fixed: Added ?. to assignee to handle unassigned tasks
